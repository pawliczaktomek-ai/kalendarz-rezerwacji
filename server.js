...
const data = res.data || { slots: [], bookings: [], smsLog: [] };
...
fs.writeFileSync(DATA_FILE, JSON.stringify({ slots: [], bookings: [], smsLog: [] }, null, 2));
}
...
// ─── Logowanie SMS ────────────────────────────────────────────────────────────
async function appendSmsLog(to, body, type) {
try {
...
to,
body,
type, // 'zawodnik' | 'trener' | 'anulowanie'
status: twilioClient ? 'wysłany' : 'testowy',
});
// Przechowuj maksymalnie 500 wpisów
if (data.smsLog.length > 500) data.smsLog = data.smsLog.slice(0, 500);
await saveData(data);
} catch (e) {
console.error('appendSmsLog błąd:', e.message);
}
}
...
// SMS do zawodnika
const playerSmsBody = `Czesc ${playerName}${playerYear ? ' (rocjnik ' + playerYear + ')' : ''}! ${eventLabel} zarezerwowany na ${dateStr}.${trainerInfo}${locationInfo} Do zobaczenia! - CSPilkarza`;
try {
await sendSMS(phone, playerSmsBody);
await appendSmsLog(phone, playerSmsBody, 'zawodnik');
} catch (e) {
console.error('SMS do zawodnika – błąd:', e.message);
await appendSmsLog(phone, playerSmsBody, 'zawodnik-błąd');
}
...
const trainerSmsBody = `Nowa rezerwacja (${eventLabel}): ${playerName} (${phone}) - ${dateStr}.${locationInfo} Wolnych miejsc: ${spotsLeft}/${maxParticipants}`;

if (trainerSmsNum) {
try {
await sendSMS(trainerSmsNum, trainerSmsBody);
await appendSmsLog(trainerSmsNum, trainerSmsBody, 'trener');
} catch (e) {
console.error(`SMS do trenera (${trainerSmsNum}) – błąd:`, e.message);
await appendSmsLog(trainerSmsNum, trainerSmsBody, 'trener-błąd');
}
...
for (const n of nums) {
try {
await sendSMS(n, trainerSmsBody);
await appendSmsLog(n, trainerSmsBody, 'trener');
} catch (e) {
console.error(`SMS do trenera (${n}) – błąd:`, e.message);
await appendSmsLog(n, trainerSmsBody, 'trener-błąd');
}
}
}
...
// SMS o anulowaniu
if (booking.phone) {
const cancelBody = `Trening anulowany przez trenera. Skontaktuj sie w celu rezerwacji nowego terminu. - CSPilkarza`;
try {
await sendSMS(booking.phone, cancelBody);
await appendSmsLog(booking.phone, cancelBody, 'anulowanie');
} catch (e) {
...
// Pobierz log SMS (admin)
app.get('/api/sms-log', requireAdmin, async (req, res) => {
const data = await loadData();
res.json(data.smsLog || []);
});

// Wyczyść log SMS (admin)
app.delete('/api/sms-log', requireAdmin, async (req, res) => {
const data = await loadData();
data.smsLog = [];
await saveData(data);
res.json({ success: true });
...
async function restoreSlots() {
  ...
  data.slots = [
    { id: uuidv4(), start: '2026-06-16T16:00:00.000Z', end: '2026-06-16T17:00:00.000Z', bookings: [], eventType: 'trening', trainer: 'Tomasz Pawliczak', location: 'Boisko SP11, ul. Spawaczy, Zielona Góra' },
    { id: uuidv4(), start: '2026-06-18T16:00:00.000Z', end: '2026-06-18T17:00:00.000Z', bookings: [], eventType: 'trening', trainer: 'Tomasz Pawliczak', location: 'Boisko SP11, ul. Spawaczy, Zielona Góra' },
    { id: uuidv4(), start: '2026-06-21T08:00:00.000Z', end: '2026-06-21T09:00:00.000Z', bookings: [], eventType: 'trening', trainer: 'Tomasz Pawliczak', location: 'Boisko SP11, ul. Spawaczy, Zielona Góra' },
    { id: uuidv4(), start: '2026-06-23T16:00:00.000Z', end: '2026-06-23T17:00:00.000Z', bookings: [], eventType: 'trening', trainer: 'Tomasz Pawliczak', location: 'Boisko SP11, ul. Spawaczy, Zielona Góra' },
    { id: uuidv4(), start: '2026-06-25T16:00:00.000Z', end: '2026-06-25T17:00:00.000Z', bookings: [], eventType: 'trening', trainer: 'Tomasz Pawliczak', location: 'Boisko SP11, ul. Spawaczy, Zielona Góra' },
    { id: uuidv4(), start: '2026-06-28T08:00:00.000Z', end: '2026-06-28T09:00:00.000Z', bookings: [], eventType: 'trening', trainer: 'Tomasz Pawliczak', location: 'Boisko SP11, ul. Spawaczy, Zielona Góra' },
    { id: uuidv4(), start: '2026-06-30T16:00:00.000Z', end: '2026-06-30T17:00:00.000Z', bookings: [], eventType: 'trening', trainer: 'Tomasz Pawliczak', location: 'Boisko SP11, ul. Spawaczy, Zielona Góra' },
    { id: uuidv4(), start: '2026-07-02T16:00:00.000Z', end: '2026-07-02T17:00:00.000Z', bookings: [], eventType: 'trening', trainer: 'Tomasz Pawliczak', location: 'Boisko SP11, ul. Spawaczy, Zielona Góra' },
    { id: uuidv4(), start: '2026-07-05T08:00:00.000Z', end: '2026-07-05T09:00:00.000Z', bookings: [], eventType: 'trening', trainer: 'Tomasz Pawliczak', location: 'Boisko SP11, ul. Spawaczy, Zielona Góra' },
  ];
