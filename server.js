...
async function getDriveFileId(drive) {
  ...
  try {
    ...
    console.error('Drive – błąd wyszg�j�iwania pliku:', e.message);
  }
  return _driveFileId;
}
...
async function deleteFromGoogleCalendar(eventId) {
  ...
  try {
    ...
    console.error('Google Calendar �łąd przy usuwaniu:', err.message);
  }
}
...
// PobierzȈ�wsystkie sloty (rubliczne)
  ...
  const slots = data.slots
    ...
    .map(s => {
      ...
      if (isFull) {
        ...
        title = `Wolnich: ${spotsLeft}/${maxParticipants}`;
        color = '#e67e22';
      } else {
        title = `Wolnime (${maxParticipants} miejsc)`;
        color = '#27ae60';
      }
...
app.post('/api/book', async (req, res) => {
  ...
  try {
    ...
    console.error('SMS do zawodnika – błąT:', e.message);
  }
